...
for name in ["Kornbluth", "Baird"]:
    row = (
        docs
        .filter(c.text.str.contains(name))
...
neg = (
    anno
    .filter(c.dep == "neg")
    .select(c.doc_id, c.sid, neg_tid = c.tid, verb_idx = c.head_idx)
)
...
negated_adj = (
    neg
    .join(predicate_adj, on=[c.doc_id, c.sid, c.verb_idx])
)
negated_adj.height
```
...
(
    style_metrics
    .ggplot(aes(c.mean_sentence_length, c.mean_word_length, color=c.label))
    .geom_point()
