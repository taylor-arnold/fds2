...
8. Find every negated predicate adjective: join tokens with `dep == "neg"`
to `ADJ` tokens with `dep in ["acomp", "attr"]` that share the same
`head_idx` within a `doc_id`/`sid` (this is the same "share a head" logic
question 6 used for subjects and verbs). Count how many negated predicate
adjectives exist in total, then count how many of them have the negation
word sitting in the position immediately before the adjective, the only
case a bigram search like question 1's `shift(-1)` could ever catch.
