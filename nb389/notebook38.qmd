...
pauses = (
    klog
    .filter(c.gap > pl.duration(seconds=2))
)
...
(
    worst_gap
    .select(c.id, c.press, c.gap, c.dur)
)
```
...
klog = (
    klog
    .with_columns(gap_ms = c.gap.dt.total_milliseconds())
)
...
(
    klog
    .select(c.gap.dt.total_milliseconds().min())
)
```
