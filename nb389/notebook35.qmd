...
leftover cookie would quietly defeat the cache on every rerun. In the first
block, write a `polite_get(url, params)` helper that clears cookies, calls
`session.get()`, raises on a bad status with `.raise_for_status()`, and
sleeps 0.2 seconds only when the response did not come from the cache. In
the second block, call it twice in a row for `"Barbara Liskov"`'s page info
and print `.from_cache` for both calls.
...
(
    laureates
    .filter(c.doc_id.str.contains(r"\("))
)
```
...
bell_labs = (
    texts
    .filter(c.text.str.contains("Bell Labs"))
)
(
    bell_labs
    .select(c.doc_id)
