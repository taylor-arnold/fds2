...
4. Build an undirected network from this edge list with
`DSNetwork.process(directed=False)`, and join the case metadata onto the node
table so the nodes are readable later. Print the node table. Then compare the
number of nodes to the 80 cases you selected, and use an anti-join to find the
...
which the `node` table already carries in the `degree` column. Sort the nodes
by degree and print the top 10 names. Then redraw the network coloring the
points by degree.
...
ones. It is in the `eigen` column, scaled so the largest score is 1. Print the
top 10 by `eigen`, and color the network by it.
...
single connected component, so filter to `component == 1` before plotting, then
color the network by `close`. Print the top 5 as well.
...
shortest paths between other cases run through a given node. Print the top 10,
this time showing `between` next to `degree` and `eigen` so the three can be
compared. Then color the network by `between`.
...
built. Color the network by `cluster`, and print each cluster's members by
grouping on `cluster` and concatenating the case names.
