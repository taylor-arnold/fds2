...
3. In the first block, re-read the first image and print the 2×2 block of
pixels in its upper-left corner (`img[:2, :2, :]`). In the second block, print
its overall mean. In the third block, print the mean of each of the three
channels separately (loop `ch` over `range(3)`).
...
fsac = (
    fsac
    .with_columns(brightness = pl.Series(brightness))
)
(
    fsac
    .select(c.filepath, c.photographer, c.brightness)
    .head(5)
)
```

5. In the first block, show the twelve darkest images in a grid with
`DSImage.plot_image_grid` (sort by `brightness`, take the head, and label the
tiles by `photographer`). In the second block, print the brightness,
photographer, and caption for the six darkest. Say what these frames have in
common.
...
(
    ...
    .ggplot(aes(c.photographer, c.brightness))
    .geom_boxplot()
    .coord_flip()
)
```
...
11. In the first block, show the twelve reddest images in a grid. In the
second block, print the caption of the six reddest. Does "red" find red
*subjects*?
