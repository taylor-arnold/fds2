...
missing = (
    text_en
    .join(text_fr.select(c.doc_id), on="doc_id", how="anti")
)
...
print(
    docs_en
    .select(c.doc_id)
    .equals(docs_fr.select(c.doc_id))
)
```
...
for author in ["Jane Austen"]:
    print(
        "EN", author,
        (
            DSText.compute_tfidf(anno_en, min_df=0.01, max_df=1.0)
            .filter(c.doc_id == author)
...
(
    anno_en
    .filter(c.token == "parser")
    .select(c.doc_id, c.token, c.upos, c.is_alpha)
    .unique()
)
```
