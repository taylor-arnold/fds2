...
8. Write data work as a pipeline, even when it is short. Wrap the whole expression in parentheses, put the DataFrame on its own line, and give every method its own line below it. This holds even when there is only one method: write the four-line form rather than `country.filter(c.region == "Europe")` on a single line. The gain is that adding or removing a step later is a one-line edit, and a reader can see the sequence of operations at a glance.
9. Avoid trailing whitespace and keep indentation consistent throughout the file.
