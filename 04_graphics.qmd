...
Every plot in this chapter is built by opening a plot from a dataset with the `.ggplot` method, in the same chained style we used for data manipulation in @sec-organize. What we cover here are geometries that draw the rows of a dataset directly: one point, label, line segment, or bar per row. Some of the most useful plots in practice, among them histograms, boxplots, density curves, and trend lines, compute something from the data before drawing it, such as a count, a percentile, or a fitted line. Those wait until @sec-graphics-ii, once we know how Polars computes group summaries; that chapter also covers facets, scales, and the finer points of labeling and theming a plot.
...
In Python, we need to use some special functions to indicate all of this information and to instruct the program to produce a plot. We start by specifying the name of the underlying dataset and chaining on the `.ggplot` method. This indicates that we want to create a data visualization. Inside that method we use the `aes` (short for aesthetics) function to specify the names of the x and y aesthetics corresponding to columns in the dataset. The plot itself is created by chaining on the `geom_point` method, using the same dot syntax we use for Polars methods. (Standard plotnine attaches layers with the plus sign; our `funs.py` file lets us chain them instead, keeping the style consistent with the rest of our code.) This method adds a point geometry (a layer of points) to the plot. Code to do this using the values described above is given below.
...
As shown in the example, we can include data-manipulation code before the call to `.ggplot`. This lets us seamlessly combine data manipulation and data visualization.
...
The point geometry requires `x` and `y` aesthetics. In addition to these required aesthetics, each geometry type also has several optional aesthetics that add information to the plot. For example, most geometries have a `color` aesthetic. The syntax for specifying this is similar to the required aesthetics, with the only difference being that we place the aesthetics inside the `geom_point()` function using another call to `aes()`. This is the convention we follow throughout the book for all aesthetics other than `x` and `y`. Let's see what happens when we add a `color` aesthetic to our scatter plot by mapping the column `subregion` to the `color` aesthetic.
...
- `color`: the color of points, lines, or the outlines of boxes
...
Although the text plot adds contextual information compared with the scatter plot, it has some shortcomings. Some labels overlap and become difficult to read, and these issues will only worsen if we increase the number of countries in our dataset. It is also unclear which part of a label corresponds to a country's HDI or happiness score: the center, the start, or the end? We could add a note that the value refers to the center of the label, but that becomes cumbersome to remember and to keep reminding others about.

To start addressing these issues, we can add the points back into the plot, with labels. We do this in Python by adding the two geometry layers (`geom_point` and `geom_text`) one after the other. This makes it clearer where each country is located on the x-axis, but it also makes the country names harder to read. To fix that, we'll add options to the text geometry to nudge the labels upward and reduce their size. Below is the code that makes both of these modifications.
...
The output of this visualization shows how the number of cell phones changes over time. The numbers generally increase slowly in North American countries. In the United Kingdom, they plateau near 2010, while in Ireland they decrease slightly from a peak around 2007.
...
country_ne = (
    country
    .filter(c.subregion == "Northern Europe")
)
(
    country
    .filter(c.region == "Europe")
