...
Load all of the modules and datasets needed for the chapter. In addition to the standard tools, we will use OpenCV (cv2) for image processing.
...
The dataset contains a `filepath` column pointing to each image file, a `label` column identifying the species, and an `index` column giving a row number that can be used to link results back to individual files. It also contains two columns of precomputed numbers, `vit` and `siglip`, which we will set aside until the second half of the chapter. This structure, with metadata in a table and actual image files stored separately, is a common pattern for working with image datasets. It allows us to use familiar tabular data tools for filtering, grouping, and summarizing while keeping the image files in their native format.
...
birds = (
    birds
    .with_columns(brightness = pl.Series(results))
)
birds
```
...
(
    ...
    .ggplot(aes(c.label, c.brightness))
    .geom_boxplot()
)
```
...
Our helper method `DSImage.compute_colors` takes an HSV image and returns a dictionary with the percentage of pixels falling into each color category. It bins the hue values into segments corresponding to common color names and also accounts for the saturation and value (very dark or desaturated pixels are grouped together into a single `neutral` category).

```{python}
DSImage.compute_colors(img_hsv)
```

The output shows what percentage of pixels in this image fall into each color category. The percentages sum to 100, giving us a complete description of the color distribution.

We will cycle through the entire DataFrame and compute these color percentages for all images.
...
(
    birds
    .select(c.filepath, c.label, c.vit)
)
```
...
(
    ...
    .ggplot(aes(c.sim_score, c.label))
    .geom_point()
)
```
