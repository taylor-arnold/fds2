...
(
    photos
    .select(rows = pl.len(), calls = c.call_number.n_unique(), files = c.filepath.n_unique())
)
```
...
word `County`: filter with `.str.ends_with`, putting the tilde operator `~` in front of the
expression to negate it (the tilde is how Polars writes "not").
